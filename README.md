...
+ International Conference on Emerging Network Experiment and Technology (CoNEXT)
...
### 2023
+ [AChecker: Statically Detecting Smart Contract Access Control Vulnerabilities](https://people.ece.ubc.ca/~mjulia/publications/ACheckerICSE2023.pdf)
...
+ [From automation to CI/CD: a comparative evaluation of threat modeling tools](https://lirias.kuleuven.be/retrieve/c334ca4e-7d9c-4eda-9a34-035181e5ca0f)
  + 무료로 사용 가능하며 실무에서 널리 활용되고 있는 상위 7개의 위협 모델링 자동화 도구(MS-TMT, pytm, CAIRIS, Threat Manager Studio, SPARTA, Threagile, TicTaaC)를 대상으로, CI/CD 방법론에 쉽게 통합될 수 있는지를 평가하였음
    + 평가 기준 1). 위협 모델링의 각 단(소스코드로부터 시스템 모델 생성, 시스템 모델로부터 위협 식별, 식별된 위협들을 입력으로 하여 위험도 평가 기반의 완화 우선순위 식별, 식별된 위협)계를 얼마나 자동화시키고 있는가?
      ...
      + pytm은 위협 라이브러리 파일에 정의된 위협별 식별 패턴과 심각도(severity: Very Low, Low, Medium, High, Very High) & 위협의 발생 가능성을 기반으로 DFD 내 위협 패턴의 존재 여부를 자동으로 식별하고, 식별된 위협에 대해 해당 파일에 매핑된 심각도와 발생 가능성을 제공함. 나머지 두 단계에 대한 자동화 기능은 일절 제공하지 않음
      + CAIRIS는 4가지 중 그 어느것도 자동화시키지 못함 (본 논문에서는 DFD를 자동 생성한다고 설명하지만, CI/CD 환경에서의 DFD 자동 생성은 소스코드를 기반으로 시스템 구조를 분석하여 생성하는 것이 핵심이다. 따라서 자산 간의 연결 관계만을 기반으로 DFD를 생성하는 방식은 본 연구의 관점에서 시스템 모델링 자동화로 보기 어렵다.)
      + Threat Manager Studio는 위협 식별 자동화 기능을 제공하는 확장 프로그램 설치시 라이브러리 파일에 정의된 위협식별 패턴에 따라 위협식별을 자동적으로 수행함. 위험도 평가의 경우, 사전 수동 설정으로 매핑된 심각도(Severity)의 값을 반영하여 전체 위험도를 자동 계산함. 나머지 두 단계에 대한 자동화 기능은 일절 제공하지 않음
      + Threagile은 위협 라이브러리 파일에 정의된 위협별 식별 패턴과 영향도도(impact) & 위협의 발생 가능성을 기반으로 DFD 내 위협 패턴의 존재 여부를 자동으로 식별하고, 식별된 위협에 대해 해당 파일에 매핑된 영행도와 발생 가능성을 기반으로 최종 심각도를 산출함. 나머지 두 단계에 대한 자동화 기능은 일절 제공하지 않음 
      + TicTaaC 은 위협 라이브러리 파일에 정의된  위협별 식별 패턴과 심각도를 기반으로 DFD 내 위협 패턴의 존재 여부를 자동으로 식별하고, 식별된 위협에 대해 해당 파일에 매핑된 심각도를 제공함. 나머지 세단계(시스템 모델링, 위험도 평가 자동화 및 완화 우선순위 식별 자동화, 위협 볗 완화방안 도출)에 대한 자동화 기능은 일절 제공하지 않음(심각도만 제공되기에 위험도 평가가 불가능함)
    + 평가 기준 3). 파이프라인 코드(예: Jenkinsfile의 코드)에서 4가지 위협 모델링 자동화 기능(시스템 모델링 자동화, 위협 식별 자동화, 위험도 평가 자동화, 완화방안 )을 호출할 수 있는가?
      + MS-TMT는 외부 호출을 위한 CLI를 제공하지 않으므로, MS-TMT 외부 도구를 통해 위협 식별 기능을 호출하는 것이 불가능함
      + pytm는 위협 식별, 위험도 평가 및 완화 우선순위 제공 기능을 위한 CLI를 제공함
      + CAIRIS는 그 어떠한 자동화 기능도 제공하지 않으나, DFD 생성은 REST API로 가능한 형태이고, DFD Export를 지원하는 python script를 제공하고 있음.   
      + Threat Manager Studio는 위협 식별 기능과 위험도 평가 기능 호출을 위한 CLI를 제공하지 않음
      + SPARTA는 위협 식별 기능과 위험도 평가 기능 호출을 위한 CLI를 제공함
      + Threagile은 위협 식별 기능과 위험도 평가 호출을 위한 CLI를 제공함 
      + TicTaaC은 위협 식별 기능 호출을 위한 CLI를 제공함
...
+ [FASR: Automated Identification of Unsafe Control Actions in STPA](https://arxiv.org/pdf/2605.30697)
  + 본 논문은 STPA(System-Theoretic Process Analysis)의 자동화를 위해 정형 기법(Formal Methods)을 활용한 FASR(Formalizing and Automating STPA with Robustness)를 제안하며, 이를 통해 UCA(Unsafe Control Action)를 자동으로 식별함
  + FASR의 입력은 두 가지로 구성됨. 첫째, SysML로 작성된 시스템 모델이며, 둘째, 시스템의 안전 요구사항을 나타내는 부울식, 출력은 분석 과정에서 식별된 UCA 목록
...
### 2026
+ [Mechanically Translating Iterative Dataflow Analysis to Algebraic Program Analysis](https://dl.acm.org/doi/pdf/10.1145/3798216)
...
+ [AsFuzzer: Differential Testing of Assemblers with Error-Driven Grammar Inference](https://dl.acm.org/doi/pdf/10.1145/3650212.3680345)
  + 상용 어셈블러는 내부 로직이나 문법 규칙이 공개되어있지 않아 블랙박스 상태로 테스트를 할수 밖에 없음. 이에 저자는 블랙박스 상태의 어셈블러에 오류가 있는지 검증하기 위한 아래의 방법론을 제안
    ...
    + 만약 독자적인 문법이 있어 다른 어셈블러와 출력 비교가 불가능한 경우에는 입력된 어셈블레 코드와 바이너리를 역어셈블한 코드가 의미상 (비교군이 없는 경우, 입력과 출력으로부터 생성한 입력 간 semantic 차이가 있는지 분석)
...
+ [Source Code-Driven GDPR Documentation: Supporting RoPA with Assessor View](https://mugdhak30.github.io/assets/Preprints/RoPA_SANER2026.pdf)
  + 본 도구의 목적은 코드 내 개인정보의 잠재적 유출 경로를 자동으로 식별·추출함으로써, 개발자가 개인정보 처리 활동과 외부 전송 현황을 정확하게 파악하고 GDPR 제13조에 따른 정보 제공 의무를 효과적으로 이행할 수 있도록 지원하는 데 있음 
  + 특정 공격 패턴(Signatures/Patterns)’을 대조하는 방식이 아니라, 사전에 식별된 출발지(Source)와 도착지(Sink) 사이에 데이터의 이동 경로가 존재하는가를 확인
    + 출발지와 도착지를 사전에 식별한 방법
      +  출발지 식별 방법 : 안드로이드 스마트폰 내부에서 민감한 정보(위치 정보, 기기 고유 ID 등)를 가져오는 기존의 알려진 안드오리드 OS API 함수들을 소스로 지정, 공개된 앱의 화면 설계도(Layout XML)를 분석하여 데이터 입력 필드의 고유 ID(저자는 공개 데이터를 분석한 결과를 바탕으로, 이 ID가 txt_name처럼 입력되는 데이터가 무엇인지를 나타내는 정보를 담고 있다고 볼 수 있다고 말함), 힌트(placeholder), 입력 타입(type) 정보를 추출한 뒤 이를 종합적으로 활용해, 해당 필드에 입력되는 데이터가 소스인지 여부를 판단
      +  도착지 식별 방법 : 외부 서버나 인터넷으로 데이터를 쏘아 보내는 통신 함수와 유명 서드파티 라이브러리(OkHttp 등)를 싱크로 지정
  + 'Do Android App Developers Accurately Report Collection of Privacy-Related Data?' 논문의 후속 논문 
+ Beyond Lexical: Functional Semantics and Fusion for Precise Architecture Recovery
+ [Do Android App Developers Accurately Report Collection of Privacy-Related Data?](https://dl.acm.org/doi/pdf/10.1145/3691621.3694949)
